...
def convolution_im2col_2_try(x:np.ndarray, kernel:np.ndarray, padding:int=0, stride:int = 1): #doesnt work because impossible to do x_pad[b_out, h_start:h_end, w_start:w_end, :] with arrays
...
t0 = time.time()
for i in range(100):
    _ = convolution_im2col_loop(x, k, padding=1, stride=1)
t1 = time.time()
