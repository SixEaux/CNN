...
class Convolutional(Layer):
    ...
    def __init__(
        self, number_kernels: int, size_kernel: int, stride: int = 1, padding: int = 0, initialization: str = "xavier"
    ):
    ...
    def initial_param(self, dim_in: tuple, dim_out: tuple):
        ...
        if self.initialization == "he":
            self.weight = (
                he_initialization(h_in * w_in, self.size_kernel * self.size_kernel * channels_in * self.number_kernels)
                .reshape((self.size_kernel, self.size_kernel, channels_in, self.number_kernels))
                .astype(np.float32)
            )
        elif self.initialization == "xavier":
            self.weight = (
                xavier_initialization(
                    h_in * w_in, h_out * w_out, self.size_kernel * self.size_kernel * channels_in * self.number_kernels
                )
                .reshape((self.size_kernel, self.size_kernel, channels_in, self.number_kernels))
                .astype(np.float32)
            )
            ...
            self.weight = np.random.uniform(
                -1, 1, (self.size_kernel, self.size_kernel, channels_in, self.number_kernels)
            ).astype(np.float32)
        else:
    ...
    def backward_filter_tensordot(self, dL_dout: np.ndarray):
        ...
        if self.padding > 0:
            x = np.pad(
                self.input, ((0, 0), (self.padding, self.padding), (self.padding, self.padding), (0, 0))
            )  # pad the image
        else:
            x = self.input
    ...
    def backward_input_tensordot(self, dL_dout: np.ndarray):
        ...
        new_h, new_w = (
            h + (h - 1) * (self.stride - 1) + 2 * pad_value,
            w + (w - 1) * (self.stride - 1) + 2 * pad_value,
        )  # h, w conversion to take stride (add zeros between rows) and padding (pad) into account
        ...
        w = np.lib.stride_tricks.sliding_window_view(
            out, (k_h, k_w), axis=(1, 2)
        )  # window view (! window of size kernel because we want to reduce by w_out, h_out and c_out in order to keep kernel shapes)
